...
* `cost_filter` - (Optional) A list of [CostFilter](#CostFilter) name/values pair to apply to budget.
* `cost_filters` - **DEPRECATED** (Optional) Map of [CostFilters](#CostFilters) key/value pairs to apply to the budget.
...
Valid key for `cost_filters` is same as `cost_filter`. Please refer to [CostFilter](#CostFilter).

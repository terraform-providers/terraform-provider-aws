...
// IotwirelessUpdateTags updates iotwireless service tags.
// The identifier is typically the Amazon Resource Name (ARN), although
// it may also be a different identifier depending on the service.
func IotwirelessUpdateTags(conn *iotwireless.IoTWireless, identifier string, oldTagsMap interface{}, newTagsMap interface{}) error {
	oldTags := New(oldTagsMap)
	newTags := New(newTagsMap)
	...
	if updatedTags := oldTags.Updated(newTags); len(updatedTags) > 0 {
		input := &iotwireless.TagResourceInput{
			ResourceArn: aws.String(identifier),
			Tags:        updatedTags.IgnoreAws().IotwirelessTags(),
		}
